...
extract_from_retour <- function(retour_path, anomalies_dt) {
  ...
  # ============================================================
  # [DEDUP EXTRACTION] - 1 ligne par ID_CONTRAT (priorité SG_CONTRACT_ID)
  # Règle:
  #   1) matched_on == "SG_CONTRACT_ID" prioritaire (priorité absolue)
  #   2) sinon ordre des autres colonnes ID
  #   3) si ASOF_DT existe: 1 ligne par (ID_CONTRAT, ASOF_DT), sinon par ID_CONTRAT
  # ============================================================
  ...
  if ("ASOF_DT" %in% names(out)) {
    setorderv(out, c("ID_CONTRAT", "ASOF_DT", "matched_prio"))
    out <- out[, .SD[1], by = .(ID_CONTRAT, ASOF_DT)]
  } else {
    setorderv(out, c("ID_CONTRAT", "matched_prio"))
